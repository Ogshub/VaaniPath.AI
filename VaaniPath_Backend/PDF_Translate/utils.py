...
def insert_text_fit(page: fitz.Page, rect, text: str, fontname: str,
    ...
    r = fitz.Rect(*rect)
    # Using my improved padding values
...
def resolve_font(logical_name, font_path=None):
    ...
    if not os.path.exists(font_path):
        # Fallback to current folder assets if not found in root
        alt_path = Path(__file__).resolve().parent.parent.parent / "assets" / "fonts" / f"{logical_name}-Regular.ttf"
        if alt_path.exists():
            return logical_name, str(alt_path)
